...
function [OUT,varargout]=pso_Trelea_vectorized_Glaze(functname,D,varargin)
...
for i=1:me  % start epoch loop (iterations)
   ...
   % this section does the plots during iterations
    if (i==1) | (rem(i,50) == 0 )
        fprintf(message,i,gbestval);  %%%% CHANGED SO THAT COMMAND LINE IS UPDATED FOR EVERY 50th ITERATION -- PM 29/08/13
    end
    if plotflg==1      
