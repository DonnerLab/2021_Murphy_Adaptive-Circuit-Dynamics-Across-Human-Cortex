...
induced = 1;
modeltype = 'fitted_linIU';  % switch b/w 'normative', 'fitted' & 'fitted_np'
...
Lenc_path = ['/mnt/homes/home024/pmurphy/Surprise_accumulation/Figs4paper/Fig5/'];
...
if strcmp(priortype,'psi'), psistr = '_psi'; else psistr = ''; end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fs(1,1:2) = [8 14];   % frequency band within which to average
fs(2,1:2) = [8 14];
fs(3,1:2) = [8 14];
...
for c = 1:length(clusters)
    ...
    for s = 1:length(allsubj)
        ...
        for cc = 1:length(clusters{c})
            % load data from all areas within current cluster (and *-1 so that LLR encoding is positive-going)
            load([loadpath,allsubj{s},'_F_',clusters{c}{cc},psistr,'_LIhalf',sstr,'_ext_beta.mat'],'TllrS_MR','smptimes','freqs')  %
            cLLR = [cLLR; squeeze(mean(mean(TllrS_MR(freqs>=fs(c,1) & freqs<=fs(c,2),:,:),1),3)).*-1];
        end
...
latwin = [0.2 1.0]; % time window (s) around sample onset to search for peak latency
latLLR=[]; fitLLR=[]; avEnc=[];
for c = 1:length(clusters)
    for subj = 1:size(gaLLR{c},2)
        ...
        m = interp1(ts,gaLLR{c}(:,subj)',newsmpts,interp_method);   % interpolate to ms resolution
        m = m(newsmpts>=latwin(1)); rts = newsmpts(newsmpts>=latwin(1));
        m = m./max(m);
        peakts = find(rts<=latwin(2));   % only search for peak latencies within first Xs of encoding function
        data_in = [rts(find(m(peakts)==max(m(peakts)),1,'first'):end)' m(find(m(peakts)==max(m(peakts)),1,'first'):end)'];  % only fitting from peak latency onward
...
xlabel('Time from sample onset (s)'), ylabel('Normalized beta (a.u.)')
...
sigx = [1.5 2.5 2]; sigy = [0.86 0.875 0.89];
...
set(gca,'FontName','Arial','LineWidth',axlw,'FontSize',fs,'TickDir','out','box','off','XTick',[],'YTick',[0.3 0.5 0.7])
