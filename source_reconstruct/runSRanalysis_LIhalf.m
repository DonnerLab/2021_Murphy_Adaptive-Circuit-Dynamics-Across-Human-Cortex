function runSRanalysis_LIhalf_ext(nsubj,nclust)
...
trlwin = [-0.5 6.2];  % window for full-trial-wise analyses
...
savepath = ['/mnt/homes/home024/pmurphy/Surprise_accumulation/Analysis/MEG/',str1,'/agg/',str2,'/'];
...
TpriorS_MR=[]; TllrS_MR=[]; TllrXsurpriseS_MR=[]; TllrXcertaintyS_MR=[]; TllrXpupilS_MR=[];
...
for s = 1:size(smp_data,4)-1  % looping through samples
    ...
    for f = 1:size(smp_data,2)  % looping through freqs
        for t = 1:size(smp_data,3)  % looping through time-points
            m = regstats(nanzscore(smp_data(:,f,t,s+1)),nanzscore([prior_full(:,s) LLR_full(:,s+1) LLR_full(:,s+1).*nanzscore(surprise_full(:,s+1)) LLR_full(:,s+1).*nanzscore(abs(prior_full(:,s))) LLR_full(:,s+1).*nanzscore(dil_full(:,s+1)) sess_r]),'linear',{'beta','tstat','rsquare'});  % signed prior, LLR, LLR*surprise & LLR*certainty
            if strcmp(coeftype,'beta')
                TpriorS_MR(f,t,s) = m.beta(2);
                TllrS_MR(f,t,s) = m.beta(3);
                TllrXsurpriseS_MR(f,t,s) = m.beta(4);
                TllrXcertaintyS_MR(f,t,s) = m.beta(5);
                TllrXpupilS_MR(f,t,s) = m.beta(6);
            elseif strcmp(coeftype,'tscore')
                TpriorS_MR(f,t,s) = m.tstat.t(2);
                TllrS_MR(f,t,s) = m.tstat.t(3);
                TllrXsurpriseS_MR(f,t,s) = m.tstat.t(4);
                TllrXcertaintyS_MR(f,t,s) = m.tstat.t(5);
                TllrXpupilS_MR(f,t,s) = m.tstat.t(6);
            end
        end
    end
end
...
% ==================================================================
% PULL PER-SAMPLE, TIME-AVERAGED SINGLE-TRIAL LATERALIZATION, Z-SCORED WITHIN SESSION
% ==================================================================
wins = 0.1:0.1:1.0;  % windows relative to sample onset over which to average
swLI = nan(size(smp_data,1),size(smp_data,2),size(smp_data,4),length(wins)-1);
for f = 1:size(smp_data,2)
    for s = 1:length(sessions)
        for smp = 1:size(smp_data,4)
            for w = 1:(length(wins)-1)
                swLI(sess_full==sessions(s),f,smp,w) = squeeze(zscore(mean(smp_data(sess_full==sessions(s),f,smptimes>=wins(w) & smptimes<=wins(w+1),smp),3)));
            end
        end
    end
end
...
if strcmp(priortype,'psi')
    if strcmp(coeftype,'beta')
        savename = [savepath,subj,'_',proc,'_',clusters{nclust},'_psi_LIhalf',strs,'_ext_beta.mat'];
    else
        savename = [savepath,subj,'_',proc,'_',clusters{nclust},'_psi_LIhalf',strs,'_ext.mat'];
    end
else
    if strcmp(coeftype,'beta')
        savename = [savepath,subj,'_',proc,'_',clusters{nclust},'_LIhalf',strs,'_ext_beta.mat'];
    else
        savename = [savepath,subj,'_',proc,'_',clusters{nclust},'_LIhalf',strs,'_ext.mat'];
    end
end

save(savename,'smptimes','freqs','wins',...
    'TpriorS_MR','TllrS_MR','TllrXsurpriseS_MR','TllrXcertaintyS_MR','TllrXpupilS_MR',...
    'LPR_full','LLR_full','surprise_full','psi_full','dil_full','swLI')
end
